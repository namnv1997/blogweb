...
class CategoryInfo
{
    ...
    public function createContentPage()
    {
        ...
        $resultSet = $post->getAllPostAboutCategory();
        $category_name = $category->getCategoryById($_REQUEST['category_id']);

        $content_area .= '<div id="content_area" class="col-md-9">';
        $content_area .= '<h3 class="color-category">' . $category_name[0]['name'] . '</h3><hr class="bottom50">';
